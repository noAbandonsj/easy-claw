...
2. 调用 `deepagents.create_deep_agent()` 创建 Agent，传入：
   ...
   - `interrupt_on={"edit_file": True, "write_file": True}`：文件写入操作触发 LangGraph interrupt
3. 通过 `ConsoleApprovalReviewer` 处理 interrupt，在控制台展示操作信息后请求用户 y/N 确认
...
`easy_claw/workflows/` 提供工具编排层，把多个工具调用串联为完整的任务流程，供 CLI 和 API 共用。

当前实现的 `document_runs.py` 工作流：

1. 调用 `load_workspace_documents` 收集和转换用户传入的文档路径
...
CLI 的 `docs summarize` 和 API 的 `POST /runs` 都复用同一个 workflow 函数 `run_document_task()`。
...
执行确认通过 LangGraph 的 Human-in-the-Loop（HITL）机制实现，不自行构建审批引擎。

当前机制（v0.2）：

1. `create_deep_agent()` 通过 `interrupt_on={"edit_file": True, "write_file": True}` 配置：当 Agent 尝试编辑或写入文件时，LangGraph 自动暂停执行并产生 interrupt
2. `ConsoleApprovalReviewer` 在控制台展示操作信息（工具名称、参数、描述），请求用户输入 y/N
3. 用户确认后，通过 `Command(resume={"decisions": [...]})` 恢复 Agent 执行
