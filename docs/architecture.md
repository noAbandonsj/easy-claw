...
- 当前缺口：LangGraph 工具结果消息不稳定携带原始调用参数；Web 端已默认折叠长工具结果，但如果 `tool_call_start` 没有参数、`tool_call_result` 也没有 `tool_call_id` 关联，就只能显示“未提供参数”。下一步需要在流式转换层记录稳定 `tool_call_id`、工具名和参数，并在结果事件中回填给 CLI/Web。
