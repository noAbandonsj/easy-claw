...
def test_chat_passes_resolved_skill_source_records(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("easy_claw.cli.LangChainAgentRuntime", FakeRuntime)
    monkeypatch.setattr(
...
def test_chat_replaces_unencodable_model_output_for_legacy_windows_console(
    tmp_path,
    monkeypatch,
):
    monkeypatch.chdir(tmp_path)
    ...
    class FakeConsole:
        ...
        def print(self, value):
            value.encode(self.file.encoding)
            printed.append(value)

    monkeypatch.setattr("easy_claw.cli.LangChainAgentRuntime", FakeRuntime)
    monkeypatch.setattr("easy_claw.cli.console", FakeConsole())
    runner = CliRunner()
...
def test_chat_interactive_uses_stream_when_session_supports_it(tmp_path, monkeypatch):
    ...
    class FakeStreamingSession:
        ...
        def stream(self, prompt, cancel_event=None, cancel_pause_event=None):
            prompts.append(prompt)
            pause_events.append(cancel_pause_event)
    ...
    assert pause_events[0] is not None
    events = [
...
def test_terminal_prompt_fallback_keeps_original_three_line_frame(monkeypatch):
    ...
    assert rendered_before_input.count("\u2500" * 16) == 2
    assert "38;5;217" in rendered_before_input
    assert "38;5;206" not in rendered_before_input
    rendered = output.getvalue()
