...
当前版本（v0.2）是 **CLI 优先 + FastAPI API 底座**，已具备本地文档总结、联网搜索、命令执行、Python 片段运行和交互式对话能力。浏览器里的 `/docs` 是接口文档，不是聊天界面；Web UI 会在后续版本实现。
...
根路径 `http://127.0.0.1:8787/` 当前返回 `Not Found` 是正常的，因为还没有 Web UI。
...
v0.2 已实现”本地个人助手 + 强工具可用性”的核心闭环，不做完整 LangGraph 长任务、桌面端、多用户、插件市场和复杂沙箱。

v0.2 已包含：

- 本地 FastAPI 服务（`/health`、`/sessions`、`/runs`）。
- SQLite 本地存储（sessions、memory_items、audit_logs 三张表）。
- `uv` 包管理：根目录 `pyproject.toml` 和 `uv.lock`。
- Typer CLI 入口（`chat`、`chat --interactive`、`docs summarize`、`tools search/run/python`、`skills list`、`memory list`、`doctor`、`init-db`、`serve`）。
- `deepagents` SDK 封装的 Agent Runtime（底层 LangChain + LangGraph）。
- Markdown Skills 加载和 DeepAgents 原生注入。
- 本地强工具：文档读取/MarkItDown 转换、DuckDuckGo 搜索、PowerShell 命令执行、Python 片段运行。
- 文档任务 Workflow 编排（`workflows/document_runs.py`）。
- 记忆：SQLite 显式记忆，保存偏好和项目笔记。
- 执行确认层：LangGraph interrupt + 控制台人工确认（文件编辑/写入）。
- `FilesystemBackend(virtual_mode=True)` 虚拟工作区边界。
- Windows 启动脚本 `scripts/start.ps1`。

v0.2 暂不包含：

- Web UI 聊天界面。
- MCP Client Adapter（真实 MCP Server 接入）。
- 多用户权限系统。
- 完整桌面客户端。
- Docker / WSL2 沙箱。
- Mem0 / Honcho 长期记忆 Provider。
- LangGraph 长任务恢复和流式输出。
