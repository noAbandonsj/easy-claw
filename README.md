...
Web 聊天页的 `/help` 使用和 CLI 相同的 slash command 定义，并支持 `/skills`、`/mcp`、`/browser`、`/sessions`、`/resume <session-id>`、`/delete-session <session-id>`、`/workspace <path>`、`/model <name>`、`/doctor` 和 `/save <path>`。Web 端的 `/workspace` 和 `/model` 只影响当前浏览器会话，不会修改 `.env`。
...
easy-claw 使用自有 Markdown skill 机制。一个 skill 是一个目录，目录中包含 `SKILL.md`，也可以带辅助脚本、模板和参考资料。运行时会通过 LangChain 工具按需列出和读取 skill，而不是一次性把全部内容塞进提示词。

当前内置技能：

- `analyze-project`：分析本地项目结构，并总结架构、命令和风险。
- `summarize-docs`：将选中的本地文档总结为简洁的 Markdown 报告。
- `create-skill`：创建、改写或检查 easy-claw 技能。
